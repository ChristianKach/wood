          ...
          <div class="">
            <div class="page-title"><br><br>
              <div class="title_center">
                <center><h3 >Résultat du Projet : <?php  echo "<b style=color:red;>".ucfirst($emb_avan->libelle)."</b>"; ?> <br><br>

                    Note de calcul de Bois: <?php 
                    if($emb_avan) { 
                                  echo "<b style=color:red;>"."Embrevement Avant"."</b>";
                                 } else {
                                  echo '';
                                 }
                                  ?>
              ...
              <?php foreach($emb_avant as $p) { 
                          ...
                          $emb_avant = $this->model_emb_avant->getById($p->emb_avant_id);
                          $pc = ($I *($emb_avant[0]->tr_emb_ar - $emb_avant[0]->tpr_emb_ar)) + (1*7);
                          $en = ($I *$emb_avant[0]->fd_emb_ar);
                          $lsi = $emb_avant[0]->lent_emb_ar - ($emb_avant[0]->face_emb_ar * $pc);
                          $hsi = $emb_avant[0]->haut_emb_ar - ($emb_avant[0]->face_emb_ar * $pc);
                          $Lsi = $emb_avant[0]->barb_emb_ar - ($emb_avant[0]->face_emb_ar * $pc);
                          $Hsi = $emb_avant[0]->harb_emb_ar - ($emb_avant[0]->face_emb_ar * $pc);                       
                          $angle = $emb_avant[0]->angle_emb_ar;               
                          $Ltal = $emb_avant[0]->ltal_emb_ar - $pc ;
                          $Htal = $emb_avant[0]->htal_emb_ar - $pc ;
                          ...
                          $Ensi = $I * $emb_avant[0]->fd_emb_ar;
                          $Cem = ($emb_avant[0]->lf_emb_ar * sqrt(12)/$Lsi);
                          $Cer = ($Cem * (sqrt($Rca/($Mma * 1000))/3.14159));
                          $Cce = (0.5 * (1 + $J * ($Cer - 0.3) + pow($Cer, 2)));
                          $Ccc = (1 / ($Cce + sqrt(pow($Cce, 2) -pow($Cer, 2))));
                          $Rco = (($Rca * $Rct * $G)/(($Rca * pow(sin(($angle/2) *(3.14159/180)), 2))+($Rct*$G*pow(cos(($angle/2) *(3.14159/180)), 2))));
            ...
            <div class="row">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                ...
                <center>
                <a href="<?php echo site_url(); ?>emb_avant/update/<?php echo $p->emb_avant_id; ?>"class="btn btn-success"><i class="fa fa-pencil"></i> Modifier</a>
                <a href="<?php echo site_url('emb_avant'); ?>" data-id="" class=" btn btn-primary"><i class="fa fa-reply"></i> Retour</a>
