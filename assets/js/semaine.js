 $(document).ready(function() {
    ...
    $('#semaine_date_range').daterangepicker({
    ...
    $('.btn_delete').click(function(e) {
        ...
        if(re) {
          window.location.href = site_url + 'semaine/delete/' + package_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les semaines sélectionnées?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer cette semaine sélectionnée?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'semaine/delete-many');
    ...
    $("#table_programme_id").change(function(e) {
          ...
          window.location.href = site_url + 'semaine/programme/' + semaine_id;
    }); 
