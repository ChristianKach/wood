 $(document).ready(function() {
    ...
    $('.btn_delete').click(function(e) {
        e.preventDefault();
        var klef_id = $(this).attr('data-id');
        var re = confirm('Êtes vous sûr de vouloir supprimer ce klef?');
        if(re) {
          window.location.href = site_url + 'klef/delete/' + klef_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les klef sélectionnés?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer ce klef sélectionné?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'klef/delete-many');
