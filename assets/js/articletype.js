 $(document).ready(function() {
    ...
    $('.btn_delete').click(function(e) {
        ...
        if(re) {
          window.location.href = site_url + 'articletype/delete/' + package_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les types article sélectionnés?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer ce type article sélectionné?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'articletype/delete-many');
