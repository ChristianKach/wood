 $(document).ready(function() {
    ...
    $('.btn_delete').click(function(e) {
        ...
        var re = confirm('Êtes vous sûr de vouloir supprimer cet coefficient de categorie?');
        if(re) {
          window.location.href = site_url + 'categorie/delete/' + categorie_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les categories sélectionnées?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer cette categorie sélectionnée?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'categorie/delete-many');
